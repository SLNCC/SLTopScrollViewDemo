...
- (void)setStaticTitleArr:(NSArray *)staticTitleArr {
    ...
    _indicatorView.y = self.frame.size.height - indicatorHeight;
...
- (void)staticTitleClick:(UITapGestureRecognizer *)tap {
    ...
    if ([self.delegate_SL respondsToSelector:@selector(SLTopTitleView:didSelectTitleAtIndex:)]) {
        [self.delegate_SL SLTopTitleView:self didSelectTitleAtIndex:index];
    }
}
